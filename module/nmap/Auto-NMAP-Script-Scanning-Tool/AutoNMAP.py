...
#---- main function ----
#-----------------------
def NMAP_XMLwithFile():
  processNMAP = subprocess.Popen(['nmap', '-Pn', args.HOST_IP, '-sV', '-sC', '-oX', 'ScanResult.xml'])
  processNMAP.wait()
  os.rename('ScanResult.xml', args.HOST_IP+'.xml')

def NMAP_XML():
  processNMAP = subprocess.Popen(['nmap', '-Pn', args.HOST_IP, '-sV', '-sC', '-oX', '-'])
  processNMAP.wait()

def NMAP_JSONwithFile(dictPortScan);
...
def PortScan():
  ...
  RawData = nmScan.scan(args.HOST_IP, arguments='-sV')
...
def BruteForcebyService(dictPortScan):
  ...
  for ip in dictPortScan.keys():
    ...
    if 'ms-sql-s' in dictPortScan[ip].keys():
    ...
    # Oracle Service
    if 'oracle' in dictPortScan[ip].keys():
    ...
    # MongoDB Service
    if 'mongodb' in dictPortScan[ip].keys():
    ...
    if 'drda' in dictPortScan[ip].keys():
...
if __name__ =="__main__":
  ...
  print ('Start Brute-Force by Service...')
  BruteResult = BruteForcebyService(PortScanResult)
  print ('Start Output JSON File...')
  NMAP_JSONwithFile(BruteResult)
  print ('Start Output JSON File...')
  NMAP_JSONwithFile(BruteResult)
  '''